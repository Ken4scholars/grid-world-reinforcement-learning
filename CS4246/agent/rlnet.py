...
from models import SLNetwork, AtariDQN
...
class ReplayBuffer():
    ...
    def push(self, transition):
        ...
        if len(self.buffer) >= self.buffer_limit:
            ...
            self.ptr += 1
            self.ptr = self.ptr % self.buffer_limit
        else:
    ...
    def sample(self, batch_size):
        ...
        states = torch.stack([torch.FloatTensor(samples[i][0]) for i in range(batch_size)]).to(device)
        actions = torch.tensor([samples[i][1] for i in range(batch_size)], device=device)
        rewards = torch.FloatTensor([samples[i][2] for i in range(batch_size)], device=device)
        next_states = torch.stack([torch.FloatTensor(samples[i][3]) for i in range(batch_size)]).to(device)
        dones = torch.FloatTensor([samples[i][4] for i in range(batch_size)])
...
#         '''
#         This function should return epsilon-greedy action.
#
#         Input:
...
def compute_loss(model, target, criterion, states, actions, rewards, next_states, dones):
    '''
    ...
    '''
    terminal = (dones > 0).flatten()
    Qm = model(states).gather(1, actions)
    Qt = target(next_states[~terminal]).detach()
    Qt_update = torch.zeros((batch_size, 1), device=device)
    Qt_update[~terminal] = rewards[~terminal] + (gamma * Qt.max(1)[0].reshape(Qt.shape[0], 1))
    Qt_update[terminal] = rewards[terminal]
    return criterion(Qm, Qt_update)


def optimize(model, target, memory, optimizer, criterion, reward):
    '''
    Optimize the model for a sampled batch with a length of `batch_size`
    '''
    batch = memory.sample(batch_size)
    loss = compute_loss(model, target, criterion, *batch)
    # print(reward)
    if reward > 0:
...
def train(model_class, env, sl_model, resume=False):
    '''
    ...
    if resume:
        ...
        model.load_state_dict(sl_model.state_dict())
    ...
    for episode in range(max_episodes):
        ...
        if len(memory) > min_buffer:
            ...
            for i in range(train_steps):
                loss = optimize(model, target, memory, optimizer, criterion, episode_rewards)
        ...
        if episode % print_interval == 0 and episode > 0:
            print(
                "[Episode {}]\tavg rewards : {:.3f},\tavg loss: : {:.6f},\tbuffer size : {},\tepsilon : {:.1f}%".format(
                    ...
                    epsilon * 100))
        if episode and episode % save_points == 0:
            save_model(model)
    return model
...
if __name__ == '__main__':
    ...
    # env = construct_random_lane_env(rewards=DenseReward)
    env = construct_random_lane_env()
    sl_model = SLNetwork(env.observation_space.shape, env.action_space.n).to(device)
    sl_epoch = 100
    sl_model.load_state_dict(torch.load("weights/epoch-" + str(sl_epoch) + ".model"))

    if args.train:
        model = train(AtariDQN, env, sl_model, resume=False)
        save_model(model)
    else:
        model = get_model()
