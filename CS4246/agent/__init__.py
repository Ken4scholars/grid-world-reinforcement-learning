...
model_path = os.path.join(script_path, 'weights/epoch-100.model')
...
def create_agent(test_case_id, *args, **kwargs):
    ...
    model_class, model_state_dict, input_shape, num_actions = 'SLNetwork', torch.load(model_path), (4, 10, 50), 5
    model = eval(model_class)(input_shape, num_actions).to(device)
    model.load_state_dict(model_state_dict)
    return ExampleAgent(test_case_id=test_case_id, model=model)
