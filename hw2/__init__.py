...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    '''
    while True:
        old_value_fn = np.copy(value_fn)
        for i in range(len(value_fn)):
            action_id = np.argmax(matmul(trn_fn[:, i, :], (rwd_fn[i] + gamma * value_fn).T))
            value_fn[i] = matmul(trn_fn[action_id, i, :], (rwd_fn[i] + gamma * value_fn).T)
            policy[i] = action_id
        if np.max(np.abs(value_fn - old_value_fn)) <= delta:
            break
