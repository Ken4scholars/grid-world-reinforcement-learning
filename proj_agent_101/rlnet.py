...
def optimize(model, target, memory, optimizer, gamma):
    '''
...
def compute_gamma(episode):
    return gamma / math.log(episode)
...
def car_speeds(prev_state, cur_state):
    ...
    for lane in range(no_lanes):
        ...
        for idx in range(len(old_car_idx)):
            ...
            if max_speeds is None or speed > max_speeds[0][lane][old_car_idx[idx]]:
                speed_mat[0][lane][new_car_idx[idx]] = speed
            else:
                speed_mat[0][lane][new_car_idx[idx]] = max_speeds[0][lane][old_car_idx[idx]]

    max_speeds = speed_mat

    return speed_mat


def train(model_class, env, sl_model, input_shape, resume=False, start_episode=0):
    '''
    ...
    for episode in range(start_episode, max_episodes):
        ...
        t_max = np.random.choice(np.arange(35, 51))
        ...
        for t in range(t_max):
            ...
            real_episode_reward += env.real_reward
            if done:
                break
        ...
        if len(memory) > min_buffer:
            if np.mean(rewards[print_interval:]) < 0.1:
                ...
                exit()
            gamma = compute_gamma(episode)
            for i in range(train_steps):
                loss = optimize(model, target, memory, optimizer, gamma)
        ...
        if episode % print_interval == 0 and episode > 0:
            print(
                "[Episode {}]\tavg rewards : {:.3f},\tsimulation rewards : {:.3f}, \tavg loss: : {:.6f},\tbuffer size : {},\tepsilon : {:.4f}%".format(
                    episode, np.mean(real_rewards), np.mean(rewards), np.mean(losses[print_interval * 10:]),
...
def test(model, env, max_episodes=600):
    ...
    for episode in range(max_episodes):
        ...
        for t in range(t_max):
            ...
            next_state, reward, done, info = env.step(action, state)
            car_speed = car_speeds(state, next_state)
            state = np.concatenate((next_state, car_speed), axis=0)
            episode_rewards += env.real_reward
            if done:
                break
        rewards.append(episode_rewards)
...
def run(train_mode=True, resume=True, start=0):
    env = get_env()
    env = EnvWrapper(env)
    input_shape = (5, env.observation_space.shape[1], env.observation_space.shape[2])
    sl_model = SLNetwork(input_shape, env.action_space).to(device)
    sl_epoch = 100
    sl_model.load_state_dict(torch.load("weights/epoch-" + str(sl_epoch) + ".model"))

    if train_mode:
        model = train(DuellingDQN, env, sl_model, input_shape, resume=resume, start_episode=start)
        save_model(model)
    else:
        model = get_model()
