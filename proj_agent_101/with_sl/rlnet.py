...
def train(model_class, env, sl_model, input_shape, resume=False):
    '''
    ...
    for episode in range(max_episodes):
        ...
        if episode % print_interval == 0 and episode > 0:
            print("[Episode {}]\tavg rewards : {:.3f},\tsimulation rewards : {:.3f}, \tavg loss: : {:.6f},\tbuffer size : {},\tepsilon : {:.1f}%".format(
                            episode, np.mean(real_rewards), np.mean(rewards), np.mean(losses[print_interval*10:]), len(memory), epsilon*100))
...
def test(model, env, max_episodes=600):
    ...
    for episode in range(max_episodes):
        ...
        for t in range(t_max):
            ...
            car_speed = speeds(state, next_state)
            state = np.concatenate((state, car_speed), axis=0)
            episode_rewards += reward
            if done:
                break
        rewards.append(episode_rewards)
...
def run(train_mode=True, resume=True):
    env = get_env()
    env = EnvWrapper(env)
    ...
    if train_mode:
        model = train(DuellingDQN, env, sl_model, input_shape, resume=resume)
        save_model(model)
    else:
        model = get_model()
