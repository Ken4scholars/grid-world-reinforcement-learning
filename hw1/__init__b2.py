...
class GeneratePDDL_Stationary:
    ...
    def generateTimeIntervals(self):
        self.time_intervals = []
        for i in range(self.width):
            self.time_intervals.append("t{}".format(i))
    ...
    def generateInitString(self):
        ...
        result = "(now {})\n".format(self.time_intervals[0])
        for i in range(self.width - 1):
            result += "(next_interval t{} t{}) ".format(i, i+1)
        result += "(next_interval t{} t{})\n".format(self.width-1, 0)
        # Setting up cars blocking at time moments
        result += "(at pt{}pt{} agent1)\n".format(self.state.agent.position.x, self.state.agent.position.y)
        for car in self.state.cars:
            for i in range(self.width):
                dx = car.position.x - i if car.position.x-i >= 0 else self.width + car.position.x - i
                result += "(blocked pt{}pt{} t{}) ".format(dx, car.position.y, i)

        result += "\n"
        # Filling up grid cells sequence
        for w in range(self.width):
            for lane in range(self.num_lanes):
                current = "pt{}pt{}".format(w, lane)
                up = lane if lane == 0 else lane-1
                down = lane if lane == self.num_lanes else lane+1
                forward = self.width-1 if w == 0 else w-1
                result += "(up_next {} pt{}pt{}) (down_next {} pt{}pt{}) (forward_next {} pt{}pt{}) "\
                    .format(current, forward, up, current, forward, down, current, forward, lane)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addPredicate(name="blocked", parameters=(("pt1", "gridcell"), ("t", "time")))
    gen.addPredicate(name="now", parameters=[("t", "time")])
    gen.addPredicate(name="next_interval", parameters=(("now", "time"), ("next", "time")), isLastPredicate=True)
    ...
    gen.addAction(name="up",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?agent) (up_next ?from ?to) (not (blocked ?to ?t2)) (now ?t1) (next_interval ?t1 ?t2))",
                  effect_string="(and (not (at ?from ?agent)) (not (now ?t1)) (at ?to ?agent) (now ?t2))")
    gen.addAction(name="down",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?agent) (down_next ?from ?to) (not (blocked ?to ?t2)) (now ?t1) (next_interval ?t1 ?t2))",
                  effect_string="(and (not (at ?from ?agent)) (not (now ?t1)) (at ?to ?agent) (now ?t2))")
    gen.addAction(name="forward",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent"), ("t1", "time"), ("t2", "time")),
                  precondition_string="(and (at ?from ?agent) (forward_next ?from ?to) (not (blocked ?to ?t2)) (now ?t1) (next_interval ?t1 ?t2))",
                  effect_string="(and (not (at ?from ?agent)) (not (now ?t1)) (at ?to ?agent) (now ?t2))")
...
def generateProblemPDDLFile(gen):
    ...
    gen.addObjects("gridcell", gen.grid_cell_list)
    gen.generateTimeIntervals()
    gen.addObjects("time", gen.time_intervals, isLastObject=True)
    gen.addInitState()
    gen.addGoalState()
