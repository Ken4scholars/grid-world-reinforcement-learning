...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        init_str += f'(at {agent_pos} agent1)\n'

        for car in self.state.cars:
            init_str += f'(blocked pt{car.position.x}pt{car.position.y}) '
        init_str += '\n'
        for w in range(self.width):
            for lane in range(self.num_lanes):
                cell = f'pt{w}pt{lane}'
                up_next = f'pt{max(w - 1, 0)}pt{max(lane - 1, 0)}'
                down_next = f'pt{max(w - 1, 0)}pt{min(lane + 1, self.num_lanes - 1)}'
                forward_next = f'pt{max(w - 1, 0)}pt{lane}'
                init_str += f'(up_next {cell} {up_next}) (down_next {cell} {down_next}) (forward_next {cell} {forward_next}) \n'
