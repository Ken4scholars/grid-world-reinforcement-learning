...
test_config = [{'lanes' : [LaneSpec(6, [-2, -2])] *2 + [LaneSpec(6, [-5, -5])] *2 +
                          [LaneSpec(5, [-4, -4])] *2 + [LaneSpec(5, [-2, -2])] *1, 'width' :50, 'seed' : 101}]
...
#                {'lanes' : [LaneSpec(3, [-1, -1])] *3,'width' :5, 'seed': 131}]
...
class GeneratePDDL_Stationary :
    ...
    def generateTimeSlots(self):
        self.time_slots = [f't{i}' for i in range(self.width)]
    ...
    def generateInitString(self) :
        ...
        '''
        init_str = ''
        agent_pos = f'pt{self.state.agent.position.x}pt{self.state.agent.position.y}'
        init_str += f'(now t0) (at {agent_pos} agent1)\n'

        init_str += ' '.join([f'(next_time t{t} t{t + 1})' for t in range(self.width - 1)]) + '\n'
        blocked = set()
        for car in self.state.cars:
            blocked_id = f'pt{car.position.x}pt{car.position.y} t0'
            blocked.add(blocked_id)
            init_str += f'(blocked {blocked_id}) '
            speed, _ = car.speed_range
            curr_pos_x = car.position.x
            for t in range(1, self.width):
                for i in range(1, abs(speed) + 1):
                    i_x = curr_pos_x - i if curr_pos_x - i >= 0 else self.width + curr_pos_x - i
                    blocked_id = f'pt{i_x}pt{car.position.y} t{t}'
                    overtake_x = i_x + 2
                    if speed == -1 and overtake_x < self.width and i_x - 1 >= 0:
                        init_str += f'(overtake pt{overtake_x}pt{car.position.y} pt{i_x - 1}pt{car.position.y} t{t}) '
                    if not blocked_id in blocked:
                        init_str += f'(blocked {blocked_id}) '
                        blocked.add(blocked_id)
                curr_pos_x = curr_pos_x + speed if curr_pos_x + speed >= 0 else self.width + curr_pos_x + speed
        ...
        for w in range(self.width):
            for lane in range(self.num_lanes):
                cell = f'pt{w}pt{lane}'
                up_next = f'pt{w - 1}pt{max(lane - 1, 0)}'
                down_next = f'pt{w - 1}pt{min(lane + 1, self.num_lanes - 1)}'
                forward_next_1 = f'pt{w - 1}pt{lane}'
                forward_next_2 = f'pt{w - 2}pt{lane}'
                forward_next_3 = f'pt{w - 3}pt{lane}'
                init_str += f'(up_next {cell} {up_next}) (down_next {cell} {down_next}) (forward_next_1 {cell} {forward_next_1}) ' \
                            f'(forward_next_2 {cell} {forward_next_2}) (forward_next_3 {cell} {forward_next_3})\n'
    ...
    def generateGoalString(self) :
        ...
        '''    
        return f'(at pt{self.state.finish_position.x}pt{self.state.finish_position.y} agent1)'
...
def generateDomainPDDLFile(gen):
    ...
    gen.addTypes(types = {"car" : None, "agent" : "car", "gridcell" : None, "timeslot": None})
    '''
    ...
    (forward_next_1 ?pt1 ?pt2) : pt2 is the next location of the car when it takes the FORWARD-1 action from pt1
    (forward_next_2 ?pt1 ?pt2) : pt2 is the next location of the car when it takes the FORWARD-2 action from pt1
    (forward_next_3 ?pt1 ?pt2) : pt2 is the next location of the car when it takes the FORWARD-3 action from pt1
    (now ?t): the current time is t
    (next_time ?t1 ?t2): time slot t2 is the next timeslot after t1 
    (overtake ?pt1 ?pt2 ?t): Moving from pt1 to pt2 at time t is an overtake
    ...
    gen.addPredicate(name="now", parameters=[("t", "timeslot")])
    gen.addPredicate(name="next_time", parameters=(("t1", "timeslot"), ("t2", "timeslot")))
    gen.addPredicate(name="overtake", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell"), ("t", "timeslot")))
    gen.addPredicate(name="blocked", parameters=[("pt1" , "gridcell"), ("t", "timeslot")] , isLastPredicate=True)
    ...
    gen.addAction(name='UP',
                  ...
                  precondition_string='(and (at ?pt1 ?car) (up_next ?pt1 ?pt2) (not (blocked ?pt2 ?t2)) (now ?t1) (next_time ?t1 ?t2))',
                  effect_string='(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (now ?t1)) (now ?t2))')

    gen.addAction(name='DOWN',
                  parameters=(('car', 'agent'), ('pt1', 'gridcell'), ('pt2', 'gridcell'), ('t1', 'timeslot'), ('t2', 'timeslot')),
                  precondition_string='(and (at ?pt1 ?car) (down_next ?pt1 ?pt2) (not (blocked ?pt2 ?t2)) (now ?t1) (next_time ?t1 ?t2))',
                  effect_string='(and (not (at ?pt1 ?car)) (at ?pt2 ?car )(not (now ?t1)) (now ?t2))')

    gen.addAction(name='FORWARD-1',
                  parameters=(('car', 'agent'), ('pt1', 'gridcell'), ('pt2', 'gridcell'), ('t1', 'timeslot'), ('t2', 'timeslot')),
                  precondition_string='(and (at ?pt1 ?car) (forward_next_1 ?pt1 ?pt2) (not (blocked ?pt2 ?t2)) (now ?t1) (next_time ?t1 ?t2))',
                  effect_string='(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (now ?t1)) (now ?t2))')

    gen.addAction(name='FORWARD-2',
                  parameters=(('car', 'agent'), ('pt1', 'gridcell'), ('pt2', 'gridcell'), ('t1', 'timeslot'), ('t2', 'timeslot')),
                  precondition_string='(and (at ?pt1 ?car) (forward_next_2 ?pt1 ?pt2) (not (blocked ?pt2 ?t2)) (now ?t1) (next_time ?t1 ?t2))',
                  effect_string='(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (now ?t1)) (now ?t2))')

    gen.addAction(name='FORWARD-3',
                  parameters=(('car', 'agent'), ('pt1', 'gridcell'), ('pt2', 'gridcell'), ('t1', 'timeslot'), ('t2', 'timeslot')),
                  precondition_string='(and (at ?pt1 ?car) (forward_next_3 ?pt1 ?pt2) (not (blocked ?pt2 ?t2)) (not (overtake ?pt1 ?pt2 ?t2)) (now ?t1) (next_time ?t1 ?t2))',
                  effect_string='(and (not (at ?pt1 ?car)) (at ?pt2 ?car) (not (now ?t1)) (now ?t2))')
...
def generateProblemPDDLFile(gen):
    ...
    gen.generateTimeSlots()
    gen.addObjects('timeslot', gen.time_slots)
