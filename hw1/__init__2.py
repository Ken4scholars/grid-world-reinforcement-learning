...
class GeneratePDDL_Stationary:
    ...
    def generateInitString(self):
        ...
        '''
        # Setting up cars to parking slots
        result = "(at pt{}pt{} agent1)\n".format(self.state.agent.position.x, self.state.agent.position.y)
        for car in self.state.cars:
            result += "(blocked pt{}pt{}) ".format(car.position.x, car.position.y)
            # result += "(at pt{}pt{} {}) (blocked pt{}pt{}) ".format(car.position.x, car.position.y, car.id, car.position.x, car.position.y)
        result += "\n"
        ...
        for w in range(self.width):
            for lane in range(self.num_lanes):
                ...
                down = lane if lane == self.num_lanes - 1 else lane+1
                forward = 0 if w == 0 else w-1
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="up",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent")),
                  precondition_string="(and (at ?from ?agent) (up_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?agent)) (at ?to ?agent))")
    gen.addAction(name="down",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent")),
                  precondition_string="(and (at ?from ?agent) (down_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?agent)) (at ?to ?agent))")
    gen.addAction(name="forward",
                  parameters=(("from", "gridcell"), ("to", "gridcell"), ("agent", "agent")),
                  precondition_string="(and (at ?from ?agent) (forward_next ?from ?to) (not (blocked ?to)))",
                  effect_string="(and (not (at ?from ?agent)) (at ?to ?agent))")
