...
class ReplayBuffer():
    ...
    def push(self, transition):
        ...
        if len(self.buffer) < self.buffer_limit:
            ...
            # cycled buffer rewriting
            self.buffer[self.pointer % self.buffer_limit] = transition
        self.pointer += 1
    ...
    def sample(self, batch_size):
        ...
        # adding batch attributes to dictionary trans
        for s in samples:
            for field in transitions.keys():
                transitions[field].append(getattr(s, field))
        # creating tensors from replay buffer sample
        states = torch.FloatTensor(np.array(transitions["state"]), device=device)
        actions = torch.LongTensor(transitions["action"], device=device)
        rewards = torch.FloatTensor(transitions["reward"], device=device)
        next_states = torch.FloatTensor(np.array(transitions["next_state"]), device=device)
        dones = torch.FloatTensor(transitions["done"], device=device)
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    '''
    not_done = torch.tensor(tuple(map(lambda s: s == 0, dones)), device=device, dtype=torch.uint8)
    done = torch.tensor(tuple(map(lambda s: s == 1, dones)), device=device, dtype=torch.uint8)
    Q = model(states).gather(1, actions)
    Q_target = target(next_states[not_done]).detach()
    Q2 = torch.zeros([batch_size, 1], device=device)
    Q2[not_done] = rewards[not_done] + (gamma * Q_target.max(1)[0].view(Q_target.shape[0], 1))
    Q2[done] = rewards[done]
    # Q2 = rewards + (gamma * Q_target.max(1)[0].view(batch_size, 1))
